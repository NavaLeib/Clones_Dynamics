...
class Model:
    ...
    def __init__(self, number_of_species, mutant_percent, number_of_mutations, net_growth_rates_array, interaction, plating_efficiency):
    ...
    def time_propagation(self, number_of_steps=10**6 , passaging = 4):
        """
        ...
        for n in range(number_of_steps):
            ...
            if (math.log2(sum(X) / self.number_of_species)) % (1) == 0 and (math.log2(sum(X) / self.number_of_species)) > 0:
                X_X[int(math.log2(sum(X) / self.number_of_species)) + flag, :] = X
                print(math.log10(n), int(math.log2(sum(X) / self.number_of_species)) + flag,
                      X[:int(self.number_of_species / 100)].sum(axis=0) / (X.sum(axis=0)))
                time_gen[int(math.log2(sum(X) / self.number_of_species)) + flag] = t
                if (math.log2(sum(X) / self.number_of_species)) % (passaging) == 0:
                    X = rng.multinomial(self.number_of_species,
                                        X / (X.sum()))  # THAT'S NOT EXACT SAMPLING _ CAN SAMPLE MORE THEN HAVE
                    print('passing every ' + str(passaging) + ' generations, ')
                    flag = flag + passaging

            if int(math.log2(sum(X) / self.number_of_species)) + flag == 99:  # no more than 100 generations to keep
                break
